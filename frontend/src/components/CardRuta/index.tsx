...
const CardRuta: React.FC<CardRutaProps> = ({ imageSrc, altText, title, group }) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Limpio Suggestion
    setSuggestions('Un momento por favor ...');
    ...
    fetch('http://localhost:3001/api/tour', {
      ...
      .then(data => {
        ...
        const cleanedSuggestions = suggestions.replace(/```html|```/g, '').trim();

        setSuggestions(cleanedSuggestions); // Asume que data tiene una propiedad suggestions
  ...
  return (
    <Card sx={{ maxWidth: 345, margin: '1rem' }}>
      ...
      <CardContent>
        ...
        <ModalRuta open={open} handleClose={handleClose} content={suggestions?suggestions:'Un momento por favor... Cargando Información ...'} />
