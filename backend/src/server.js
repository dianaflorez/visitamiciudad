...
server.post('/api/tour', async (req, res) => {
  ...
  try {
    ...
    const prompt = `Sugiere rutas turísticas en Pasto para un grupo ${group} que estará ${days} días, ademas no olvides incluir indicaciones descriptivas paso a paso y direcciones de como llegar y el resultado envialo como codigo html pero sin la cabecera html ni el body solo el contenido donde cada dia sea un titulo en nedrita y despues de cada sugerencia por dia haya un espacio si puedes agregar  enlaces de la ubicacion geografica de google maps que si los anexas deben abrir en otra pagina mejor `;
