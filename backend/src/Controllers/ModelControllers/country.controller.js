...
const CountryController = {
    ...
    get: async () => {
        try {
            const attributes = Object.keys(Country.getAttributes());
    ...
    // Obtener un Country por ID
        getById: async (id) => {
            try {
                ...
                return responseF({ status: 200, success: true, data: country });
            } catch (error) {
    ...
    put: async ( id, newData ) => {
        try {
            ...
            await country.update(dataToUpdate);
            
            // Devolver el registro actualizado
            return responseF({ status: 200, success: true, message: 'Country actualizado exitosamente', data: country });
        } catch (error) {
