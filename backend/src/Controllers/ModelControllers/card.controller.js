  ...
  for (const [key, value] of Object.entries(foreignKeys)) {
    if (value) {
      const associatedModel =
        db.models[card.getAttributes()[key].references.model];
...
const cardController = {
  ...
  get: async () => {
    try {
      ...
      return responseF({
        status: 500,
        message:
          "Error interno al tratar de obtener los títulos de las columnas",
        success: false,
      });
    }
  },
  ...
  getById: async (id) => {
    try {
      ...
      return responseF({
        status: 500,
        success: false,
        message: "Error interno al tratar de obtener el card",
      });
    }
  },
    ...
    try {
      ...
      if (orderField) {
        if (orderField === "card_group")
          ...
          order = [
            [{ model: db.models["usuario"], as: "created" }, "", orderType],
          ];
        ...
        else order = [[orderField, orderType]];
      } else {
        order = undefined;
      }
      ...
      return responseF({
        status: 500,
        success: false,
        message: `Error interno al tratar de obtener ${
          filters ? ` card por filtro ` : `todos los registros de card`
        }`,
      });
    }
  },
  ...
  put: async (id, newData) => {
    try {
      ...
      const foreignKeyValidationResult = await validateForeignKeys(
        newData.foreignKeys
      );
      if (foreignKeyValidationResult?.error)
        return responseF({
          status: 400,
          success: false,
          message: "Error de validación de llave foránea",
        });
      ...
      return responseF({
        status: 500,
        success: false,
        message: `Error en el servidor al actualizar el card con id: ${id}`,
      });
    }
  },
};
